...
echo "</div>";
 echo "<x href=\"$PHP_SELF\" target=\"_blank\" class=\"small-box-footer\">Details <i class=\"fa fa-arrow-circle-right\"></i></a>";
 echo "</div>";
echo "</div><!-- ./col -->";
...
$countTotal = getDetail($conn, "select count(bugNumber) as value from (SELECT bugNumber,ldapManagerName from bugDescriptions
            ...
            echo "<div class=\"col-lg-3 col-xs-6\">";
              ...
              echo "<div class=\"small-box bg-yellow\">";
                ...
                echo "</div>";
                echo "<a href=\"printDetail.php?action=printTotalCRs&releaseName=$releaseName&targetRelease=$targetRelease\" target=\"_blank\" class=\"small-box-footer\">Details <i class=\"fa fa-arrow-circle-right\"></i></a>";
              echo "</div>";
            echo "</div><!-- ./col -->";
...
$countAssigned = getDetail($conn, "SELECT count(bugNumber) as value from bugDescriptions
            ...
            echo "<div class=\"col-lg-3 col-xs-6\">";
              ...
              echo "<div class=\"small-box bg-red\">";
                ...
                echo "</div>";
                echo "<a href=\"printDetail.php?action=printOpenCRs&releaseName=$releaseName&targetRelease=$targetRelease\" target=\"_blank\" class=\"small-box-footer\">Details <i class=\"fa fa-arrow-circle-right\"></i></a>";
              echo "</div>";
            echo "</div><!-- ./col -->";
...
$countClosed = getDetail($conn, "SELECT count(bugNumber) as value from bugDescriptions
            ...
            echo "<div class=\"col-lg-3 col-xs-6\">";
              ...
              echo "<div class=\"small-box bg-green\">";
                ...
                echo "</div>";
                echo "<a href=\"printDetail.php?action=printClosedCRs&releaseName=$releaseName&targetRelease=$targetRelease\" target=\"_blank\" class=\"small-box-footer\">Details <i class=\"fa fa-arrow-circle-right\"></i></a>";
              echo "</div>";
            echo "</div><!-- ./col -->";
...
$mgrName = array
  ...
  array("Raj Velusamy",0,0,0,0),
  array("Shankar Palanivel",0,0,0,0),
...
for ($row = 0; $row <= 4; $row++) {
  for ($col = 1; $col <= 1; $col++) {
   ...
   $sql = "SELECT count(bugNumber) as value from (SELECT bugNumber,ldapManagerName  from bugDescriptions LEFT JOIN releaseTracking USING(bugNumber) LEFT JOIN users ON bugDescriptions.creator=users.username WHERE (releaseDetected='$releaseName'  || targetReleaseId='$targetRelease') AND ldapManagerName like '%$ldapName%' GROUP BY bugNumber) as t1";
...
for ($row = 0; $row <= 4; $row++) {
  for ($col = 1; $col <= 1; $col++) {
   $ldapName = $mgrName[$row][0];
   $sql = "SELECT count(bugNumber) as value from bugDescriptions LEFT JOIN releaseTracking USING(bugNumber) LEFT JOIN users ON bugDescriptions.creator=users.username WHERE (((((globalState='Assigned' || globalState='Idle') AND releaseDetected='$releaseName' AND (isNull(releaseTracking.releaseState))) || (globalState='Assigned Review' || globalState='Study' AND releaseDetected='$releaseName') || (globalState='Assigned' AND releaseDetected='$releaseName' AND releaseTracking.targetReleaseId <> '$targetRelease')) OR (((releaseState='Assigned' AND globalState='Assigned') || releaseState='Build Pending' || releaseState='Committing' || releaseState='Assigned Review') AND targetReleaseId='$targetRelease'))) AND ldapManagerName like '%$ldapName%' GROUP BY ldapManagerName";
...
for ($row = 0; $row <= 4; $row++) {
  for ($col = 1; $col <= 1; $col++) {
   $ldapName = $mgrName[$row][0];
   $sql = "SELECT count(bugNumber) as value from bugDescriptions LEFT JOIN releaseTracking USING(bugNumber) LEFT JOIN users ON bugDescriptions.creator=users.username WHERE (((releaseState='Verified' || releaseState='Task Complete' || releaseState='Released' || releaseState='Unverified Released') AND targetReleaseId='$targetRelease') OR ((globalState =  'Closed' || globalState =  'Duplicate' || globalState =  'No Change' || globalState =  'Deferred') AND releaseDetected='$releaseName')) AND ldapManagerName like '%$ldapName%' GROUP BY ldapManagerName";
...
for ($row = 0; $row <= 4; $row++) {
  for ($col = 1; $col <= 1; $col++) {
   $ldapName = $mgrName[$row][0];
   $sql = "SELECT count(bugNumber) as value from bugDescriptions LEFT JOIN releaseTracking USING(bugNumber) LEFT JOIN users ON bugDescriptions.creator=users.username WHERE (((releaseState='Verify Fix' || releaseState='Verify Task Complete') AND targetReleaseId='$targetRelease')  OR ((globalState='Feedback Needed' || globalState='Verify Duplicate' || globalState='Verify No Change' || globalState='Verify Fix') AND releaseDetected='$releaseName')) AND ldapManagerName like '%$ldapName%' GROUP BY ldapManagerName";
